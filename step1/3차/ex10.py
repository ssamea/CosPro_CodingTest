...
class Shop:
    def __init__(self):
        self.reserve_list = [] #가게에 예약한 고객 리스트

#고객을 매개변수로 받아, 예약 고객 리스트에 추가 후 true를 return합니다.
...
class Restaurant(Shop):
    ...
    def reserve(self, customer):
        if customer.num_of_people < 2 and customer.num_of_people > 8: #인원수가 2명 이상 8명 이하인 경우가 아니면 false
            return False
        count = 0
        for r in self.reserve_list:
            if r.time == customer.time:
